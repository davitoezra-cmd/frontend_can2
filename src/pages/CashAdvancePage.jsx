...
import CashAdvanceStats from '../cashAdvance/CashAdvanceStats';
import CashAdvanceFilter from '../cashAdvance/CashAdvanceFilter';
import CashAdvanceTable from '../cashAdvance/CashAdvanceTable';
import CashAdvanceModal from '../cashAdvance/CashAdvanceModal';
import CashAdvanceDetailModal from '../cashAdvance/CashAdvanceDetailModal';
...
const CashAdvancePage = () => {
  ...
  const fetchCashAdvances = async () => {
    try {
      setLoading(true);
      const response = await apiFetch.get('/employee/cash-advance');
      if (response.data?.success) {
        setCashAdvances(response.data.data || []);
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Gagal mengambil data kasbon.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDetail = async (id) => {
    try {
      const response = await apiFetch.get(`/employee/cash-advance/${id}`);
      if (response.data?.success) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      text: 'Apakah Anda yakin ingin menghapus pengajuan kasbon ini?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        try {
          const response = await apiFetch.delete(`/employee/cash-advance/${id}`);
          if (response.data?.success) {
            toast.success(response.data.message || 'Pengajuan kasbon berhasil dihapus.');
            fetchCashAdvances();
          }
        } catch (error) {
  ...
  const filteredData = cashAdvances.filter((item) => {
    const matchesSearch = item.reason ? item.reason.toLowerCase().includes(search.toLowerCase()) : true;
  ...
  return (
    <div className="min-vh-100 position-relative" style={{ backgroundColor: '#F8FAFC' }}>
      ...
      {/* Area Konten Utama Halaman Kasbon */}
      <div className="main-content-wrapper d-flex flex-column min-w-0">
        ...
        <main className="container-fluid p-3 p-md-4 flex-grow-1 overflow-auto" style={{ maxWidth: '1600px' }}>
          ...
          <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            <div>
              <h3 className="fw-bold text-dark mb-1 fs-4 fs-md-3">Pengajuan Kasbon</h3>
              <p className="text-muted mb-0 small">Ajukan kasbon dan pantau status persetujuan dari admin.</p>
            </div>
            <button
            ...
            >
              <FaPlus /> <span>Ajukan Kasbon</span>
            </button>
          </div>
